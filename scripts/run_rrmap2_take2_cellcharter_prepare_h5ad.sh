...
INPUT="${1:-/Volumes/moldiassd/RRMAP2_xenium_adata/kmeans_separated/RRMAP2_xenium_all_samples.annotated.filtered.processed.cellcharter.take2.h5ad}"
OUTPUT_H5AD="${2:-/Volumes/moldiassd/RRMAP2_xenium_adata/kmeans_separated/RRMAP2_xenium_all_samples.annotated.filtered.processed.cellcharter.take2.companion.ready.h5ad}"

# Large Xenium 5k dataset (~1.38M cells, 54 samples). X is already
...
# New metadata columns (condition, sex, model, region, stage, sample_name,
# meta_sample_id) are available as colors automatically; analytics stay on the
# CellCharter/leiden clusterings.
